...
class AuxLoader:
    ...
    def __init__(self, parquet_folder=Pipeline.STORAGE_AUX, table_name=None, json_path=Pipeline.JSON_PATH):
    ...
    def read_parquet_files(self):
        ...
        if not all_files:
            raise Exception(f"Nenhum arquivo .parquet encontrado na pasta {self.parquet_folder}.")
...
def load_dim_tables():
    """Função para carregar todas as tabelas dimensionais de uma vez."""
    tables = [
        Pipeline.TABLE_D_BLOCOS,
        Pipeline.TABLE_D_NCM,
        Pipeline.TABLE_D_NCM_ISIC,
        Pipeline.TABLE_D_NCM_SH,
        Pipeline.TABLE_D_PAIS,
        Pipeline.TABLE_D_UF,
        Pipeline.TABLE_D_UNIDADE,
        Pipeline.TABLE_D_URF,
        Pipeline.TABLE_D_VIA
    ]

    for table in tables:
        print(f"\nIniciando carga para a tabela {table}")
        loader = ParquetToPostgresLoader(
            parquet_folder=Pipeline.STORAGE_EXP,  # ajuste se precisar outra pasta
            table_name=table
        )
        loader.run()
        print(f"Finalizada carga para a tabela {table}")